...
cc -nostdlib -ffreestanding -fno-builtin -fno-stack-protector -fPIE -c \
  "$HERE/src/rdr_wrapper_v19.c" -o "$BUILD/rdr_wrapper_v19.o"

cc -nostdlib -pie -Wl,-e,_start \
  -Wl,--dynamic-linker="$ROOT/runtime/bin/linker64" \
  -Wl,-rpath,"$ROOT/runtime/lib64:$ROOT/game/lib" \
  -Wl,--no-as-needed "$BUILD/rdr_wrapper_v19.o" \
  -L"$STUB" -ldl -lc -o "$BUILD/rdr_wrapper_v19"

patchelf --add-needed libm.so "$BUILD/rdr_wrapper_v19"
echo "built $BUILD/rdr_wrapper_v19"
