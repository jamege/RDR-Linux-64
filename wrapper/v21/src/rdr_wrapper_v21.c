...
static int wrapper_main(void)
{
    ...
    out("=== RDR Bionic wrapper v21 ===\n");
    out("[v21] disabling Bionic heap tagging...\n");
    if (mallopt(-204, 0)) out("[v21] heap tagging disabled\n");
    else out("[v21] WARNING: heap-tagging request was rejected\n");
    ...
    if (!create_namespace || !android_dlopen_ext || !link_namespaces) {
        out("[v21] Android namespace API unavailable\n");
        return 20;
    }

    out("[v21] creating isolated game namespace...\n");
    ns = create_namespace("rdr_game", SEARCH_PATH, SEARCH_PATH,
        ANDROID_NAMESPACE_TYPE_ISOLATED, SEARCH_PATH, 0);
    if (!ns) {
        out("[v21] namespace creation failed: "); out(dlerror()); out("\n");
        return 21;
    }

    out("[v21] sharing core Bionic libraries from default namespace...\n");
    if (!link_namespaces(ns, 0,
            "libc.so:libdl.so:libm.so:libdl_android.so")) {
        out("[v21] namespace link failed: "); out(dlerror()); out("\n");
        return 22;
    }

    ext.flags = ANDROID_DLEXT_USE_NAMESPACE;
    ext.library_namespace = ns;
    out("[v21] loading RDR and packaged libc++ in one namespace...\n");
    if (!android_dlopen_ext(RDR_PATH, RTLD_NOW, &ext)) {
        out("[v21] RDR load failed: "); out(dlerror()); out("\n");
        return 30;
    }
    out("[v21] RDR loaded successfully\n");
    return 0;
}
